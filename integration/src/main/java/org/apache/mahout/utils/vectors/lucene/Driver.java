...
public final class Driver {
  ...
  public void dumpVectors() throws IOException {
    ...
    try {
      ...
      if(mapOut != null){
    	  File map = new File(mapOut);
    	  Writer fWriter = Files.newWriter(map, Charsets.UTF_8);
    	  RecNumDocIdMapFileWriter mapWriter =  new RecNumDocIdMapFileWriter(fWriter, "\t");
    	  try{
    		  mapWriter.write(iterable, numDocs);
    	  } finally {
    		  mapWriter.close();
    	  }
      }
  ...
  public static void main(String[] args) throws IOException {
    ...
    Option docNumberIdMapperOpt = obuilder.withLongName("docIdMapOutput").withRequired(false).withArgument(
    	abuilder.withName("docIdMapOutput").create()).withDescription(
    	"The absolute path of the output location to place the document id and document number mapping").withShortName("mo").create();
    ...
    Group group = gbuilder.withName("Options").withOption(inputOpt).withOption(idFieldOpt).withOption(
        ...
        .withOption(weightOpt).withOption(minDFOpt).withOption(maxPercentErrorDocsOpt).withOption(docNumberIdMapperOpt).create();
    ...
    try {
      ...
      if (cmdLine.hasOption(inputOpt)) { // Lucene case
        ...
        if(cmdLine.hasOption(docNumberIdMapperOpt)){
        	luceneDriver.setMapOut(cmdLine.getValue(docNumberIdMapperOpt).toString());
        }
  ...
  public void setMapOut(String mapOut){
	  this.mapOut = mapOut;
  }
